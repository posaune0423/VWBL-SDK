...
export class VWBLBase {
   ...
   */
  protected _sign = async (signer: WalletClient, targetContract?: string) => {
    if (!signer.chain) throw new Error('Chain is not defined')
  ...
  protected _getAddressBySigner = async (signer: WalletClient): Promise<string> => {
    if (!signer.account) throw new Error('Account is not defined')
    return `0x${signer.account.address}`
  }
   ...
   * @returns Encrypted file data
   */
  decryptStream = (encryptFile: ReadableStream<Uint8Array>, key: string): NodeJS.ReadableStream => {
