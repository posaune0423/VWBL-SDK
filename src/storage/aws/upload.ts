...
export const uploadThumbnail = async (
  ...
  const s3Client = new S3Client({ credentials })
  const fileName =
    thumbnailImage instanceof File ? thumbnailImage.name : thumbnailImage.split('/').slice(-1)[0] //ファイル名の取得だけのためにpathを使いたくなかった
  ...
  const uploadCommand = new PutObjectCommand({
    ...
    Key: key,
    Body:
      thumbnailImage instanceof File ? thumbnailImage : await fs.promises.readFile(thumbnailImage),
    ContentType: type,
    ACL: 'public-read',
  })
